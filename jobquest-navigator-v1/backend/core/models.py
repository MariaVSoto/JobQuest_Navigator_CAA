...
class Company(BaseModel):
    ...
    # AI Research Data (Epic 6) - Enhanced with status tracking and metadata
    ai_research_data = models.JSONField(
        null=True,
        blank=True,
        help_text="Stores the structured company research data from the AI service, matching the CompanyResearch schema."
    )
    ai_research_model = models.CharField(
        max_length=50,
        null=True,
        blank=True,
        help_text="The AI model used for the last research generation (e.g., gpt-4o)."
    )
    ai_research_prompt_version = models.CharField(
        max_length=50,
        null=True,
        blank=True,
        help_text="The prompt version used for the last research generation (e.g., company_research_v1)."
    )
    ai_research_generated_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text="Timestamp of the last successful research generation."
    )
    ai_research_status = models.CharField(
        max_length=10,
        choices=[
            ('PENDING', 'Pending'),
            ('COMPLETED', 'Completed'),
        ...
        default='NONE',
        db_index=True,
        help_text="The current status of the AI research generation for this company."
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['ai_research_status']),
        ]

    def __str__(self):
        return self.name

    def update_ai_research_timestamp(self):
        """Update the AI research timestamp."""
        self.ai_research_generated_at = timezone.now()
        self.save(update_fields=['ai_research_generated_at'])
