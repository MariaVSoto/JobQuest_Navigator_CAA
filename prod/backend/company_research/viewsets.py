"""
ViewSets for Epic 6: Company Research & Interview Preparation.
...
class CompanyResearchViewSet(viewsets.ModelViewSet):
    """
    ViewSet for managing company research.
    Provides CRUD operations, research generation, and user-specific filtering.
    """
    ...
    def get_queryset(self):
        """Filter research to authenticated user's data."""
        return CompanyResearch.objects.filter(user=self.request.user).select_related('company')

    def get_serializer_class(self):
        """Use summary serializer for list view, full serializer for detail operations."""
        if self.action == 'list':
    ...
    def generate(self, request):
        ...
        POST /company-research/generate/
        """
    ...
    @drf_action(detail=True, methods=['delete'])
    def unsave(self, request, pk=None):
        """
        Unsave a company research item.
        DELETE /company-research/{id}/unsave/
        """
        research = self.get_object()
        ...
        except SavedResearch.DoesNotExist:
            return Response(
    ...
    def saved(self, request):
        ...
        GET /company-research/saved/
        """
        saved_research = SavedResearch.objects.filter(user=request.user).select_related(
            'company_research__company'
        )
        
        page = self.paginate_queryset(saved_research)
        if page is not None:
            serializer = SavedResearchSerializer(page, many=True)
            return self.get_paginated_response(serializer.data)
        
        serializer = SavedResearchSerializer(saved_research, many=True)
        return Response(serializer.data)


class InterviewPreparationViewSet(viewsets.ModelViewSet):
    """
    ViewSet for managing interview preparation materials.
    Provides CRUD operations and preparation generation.
    """
    ...
    def get_queryset(self):
        """Filter to user's interview preparations."""
    ...
    def generate(self, request):
        ...
        POST /interview-preparation/generate/
        """
    ...
    def mark_reviewed(self, request, pk=None):
        """
        Mark preparation as reviewed and update last_reviewed timestamp.
        POST /interview-preparation/{id}/mark_reviewed/
        """
        preparation = self.get_object()
        preparation.last_reviewed = timezone.now()
        preparation.save(update_fields=['last_reviewed'])
        
        return Response({
            'message': 'Preparation marked as reviewed',
            'last_reviewed': preparation.last_reviewed
        })
...
class InterviewQuestionViewSet(viewsets.ModelViewSet):
    ...
    Provides CRUD operations, filtering, and question generation.
    """
    permission_classes = [permissions.IsAuthenticated]
    pagination_class = StandardResultsSetPagination
    filter_backends = [DjangoFilterBackend, filters.SearchFilter]
    ...
    def get_serializer_class(self):
        """Use list serializer for list view, full serializer for detail operations."""
        if self.action == 'list':
    ...
    def rate(self, request, pk=None):
        ...
        POST /interview-questions/{id}/rate/
        """
        ...
        # Update question statistics (simplified implementation)
        question.times_used += 1
        current_total = question.average_rating * (question.times_used - 1)
        question.average_rating = (current_total + int(rating)) / question.times_used
        question.save(update_fields=['times_used', 'average_rating'])
...
class PracticeSessionViewSet(viewsets.ModelViewSet):
    """
    ViewSet for managing interview practice sessions.
    ...
    def get_queryset(self):
        """Filter to user's practice sessions."""
    ...
    def complete(self, request, pk=None):
        """
        Complete a practice session and save final data.
        ...
        session.questions_attempted = request.data.get('questions_attempted', 0)
        session.self_rating = request.data.get('self_rating')
        session.notes = request.data.get('notes', '')
        session.areas_for_improvement = request.data.get('areas_for_improvement', '')
        
        # Update session data with final information
        if 'session_data' in request.data:
            session.session_data.update(request.data['session_data'])
    ...
    def recent(self, request):
        ...
        GET /practice-sessions/recent/
        """
        recent_sessions = self.get_queryset()[:5]
        serializer = PracticeSessionSerializer(recent_sessions, many=True)
        return Response(serializer.data)


class CompanyInsightViewSet(viewsets.ModelViewSet):
    """
    ViewSet for managing company insights.
    Provides CRUD operations and insight management.
    """
    ...
    def upvote(self, request, pk=None):
        """
        Upvote a company insight.
        POST /company-insights/{id}/upvote/
        """
        insight = self.get_object()
        insight.upvotes += 1
        insight.save(update_fields=['upvotes'])
        
        return Response({
            'message': 'Insight upvoted',
    ...
    def downvote(self, request, pk=None):
        """
        Downvote a company insight.
        POST /company-insights/{id}/downvote/
        """
        insight = self.get_object()
        insight.downvotes += 1
        insight.save(update_fields=['downvotes'])
        
        return Response({
            'message': 'Insight downvoted',
            'upvotes': insight.upvotes,
            'downvotes': insight.downvotes
        })
...
class CompanyNewsViewSet(viewsets.ReadOnlyModelViewSet):
    """
    ViewSet for reading company news articles.
    Provides read-only access to company news.
    """
    ...
    def recent(self, request):
        ...
        GET /company-news/recent/
        """
        recent_news = self.get_queryset()[:10]
        serializer = CompanyNewsSerializer(recent_news, many=True)
        return Response(serializer.data)