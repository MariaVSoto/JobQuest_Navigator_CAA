...
class ResumeRestoreVersionView(APIView):
    """Restore a resume to a specific version (CBV version)"""
    permission_classes = [permissions.IsAuthenticated]
...
class ResumeDownloadExportView(APIView):
    """Download a resume export file (CBV version)"""
    permission_classes = [permissions.IsAuthenticated]

    def post(self, request, resume_id, export_id):
        resume = get_object_or_404(Resume, id=resume_id, user=request.user)
...
# Utility Views
class ResumeCloneView(APIView):
    """Clone an existing resume (CBV version)"""
    permission_classes = [permissions.IsAuthenticated]
...
class ResumeSetDefaultView(APIView):
    """Set a resume as the default resume (CBV version)"""
    permission_classes = [permissions.IsAuthenticated]
...
class ResumeAnalyticsView(APIView):
    """Get analytics for user's resumes (CBV version)"""
    permission_classes = [permissions.IsAuthenticated]
