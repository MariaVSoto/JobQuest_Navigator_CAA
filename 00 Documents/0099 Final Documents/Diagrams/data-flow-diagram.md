# Data Flow Diagram – JobQuest Navigator

## Overview
This document describes the data flow for the JobQuest Navigator platform, including how data moves between services, databases, and external APIs.

## Main Data Flows
- **Job Data:** Fetched from Adzuna API and Google Maps, processed by the Job Service, and stored in MySQL.
- **Resume Data:** Managed by the Resume Service, stored in MySQL, and sent to the AI Suggestion Service for analysis.
- **AI Suggestions:** Generated by the AI Suggestion Service using OpenAI API, results are returned to the Resume Service and user interface.
- **Certification Data:** Managed by the Certification Service, with market demand alerts integrated from external sources.
- **Company Research:** Company insights and interview questions are generated by the Company Research Service, using both internal data and OpenAI API.
- **User Data:** Managed by the User Service, including authentication and profile information.

## Diagram
*(See attached image for the latest data flow diagram.)*
